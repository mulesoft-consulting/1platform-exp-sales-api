pipeline {
  ...
  environment {
    ...
    WORKER = "Small"
    APP_CLIENT_CREDS = credentials("$BRANCH_NAME-api-mgr-exp-sales-api")
  }
  stages {
    stage('Prepare') {
      steps {
        configFileProvider([configFile(fileId: "${BRANCH_NAME}-exp-sales-api.yaml", replaceTokens: true, targetLocation: './src/main/resources/config/configuration.yaml')]) {
    ...
    stage('Deploy Development') {
      ...
      environment {
        ...
        APP_NAME = 'dev-nto-sales-order-api-v1'
      }
      steps {
        withMaven(
          mavenSettingsConfig: 'f007350a-b1d5-44a8-9757-07c22cd2a360'){
    ...
    stage('Deploy Production') {
        ...
        environment {
          ...
          APP_NAME = 'nto-sales-order-api-v1'
        }
        steps {
          withMaven(
            mavenSettingsConfig: 'f007350a-b1d5-44a8-9757-07c22cd2a360'){
